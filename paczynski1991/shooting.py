...
def get_Z_match_from_in(j_star):
    ...
    res = solve_ivp(get_dZ_dx, [x_in, x_match], get_Z_in(), t_eval=[x_match], args=[j_star], method=method)
...
def get_Z_match_from_out(j_star):
    ...
    res = solve_ivp(get_dZ_dx, [x_out, x_match], get_Z_out(j_star), t_eval=[x_match], args=[j_star], method=method)
