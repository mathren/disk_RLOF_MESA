...
struct Solvde {
	...
	Solvde(const Int itmaxx, const Doub convv, const Doub slowcc,
		...
		MatDoub_IO &yy, R &difeqq, Int &convtestt);
...
template <class R>
Solvde<R>::Solvde(const Int itmaxx, const Doub convv, const Doub slowcc,
	VecDoub_I &scalvv, VecInt_I &indexvv, const Int nbb, MatDoub_IO &yy,
	R &difeqq, Int &convtestt) : itmax(itmaxx), conv(convv), slowc(slowcc),
	scalv(scalvv), indexv(indexvv), nb(nbb), y(yy), convtest(convtestt), difeq(difeqq),
	...
	s(ne,2*ne+1)
{
	...
	for (Int it=0;it<itmax;it++) {
		...
		err /= nvars;
		Doub fac=(err > slowc ? slowc/err : 1.0); // disable slowc
		for (Int j=0;j<ne;j++) {
			jv=indexv[j];
			for (k=k1;k<k2;k++)
			y[j][k] -= fac*c[jv][0][k];
		...
		if (err < conv) { //Set convest =0 if it converged
			convtest=0;
			//cout << 0 << endl;
			return;}
	}
...
template <class R>
void Solvde<R>::pinvs(const Int ie1, const Int ie2, const Int je1, const Int jsf,
	const Int jc1, const Int k)
{
...
template <class R>
void Solvde<R>::bksub(const Int ne, const Int nb, const Int jf, const Int k1,
	const Int k2)
{
	Int nbf=ne-nb,im=1;
...
template <class R>
void Solvde<R>::red(const Int iz1, const Int iz2, const Int jz1, const Int jz2,
	const Int jm1, const Int jm2, const Int jmf, const Int ic1,
	const Int jc1, const Int jcf, const Int kc)
{
	Int l,j,i;
